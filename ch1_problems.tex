...
\frametitle{Problems}
\begin{enumerate}
  \item[7] For a Discrete Memoryless Source (DMS) $X=\{a,b,c\}$ with probability distributions
  \begin{itemize}
...
\begin{enumerate}
   ...
   \pause \item[12] Formulate conditions of $\alpha$ and $\beta$ such that the Markov chain with the probability transition matrix
    \[
    \Pi=\left[%
    \begin{array}{cc}
    \alpha & 1-\alpha \\
...
\begin{enumerate}
  \item[16]     
    \footnotesize{
      ...
      Consider BMS with $p($ "1" $)=p$, find the probability distribution for the lengths of series $Y$ and entropy $H(Y)$.  
...
\begin{enumerate}
  ...
  Why is it called so? Verify the probability normalization condition. Find mathematical expectation, variation and entropy $H(I)$.
\end{enumerate}
\end{frame}
...
\begin{enumerate}
  ...
  Prove that a
  \emph{Geometric distribution is entropy-maximizing for discrete random variables having fixed mathematical expectation.}
...
\begin{enumerate}
  ...
  \pause \item[21] Consider DMS from the previous problem. Investigate the dependency between code rate of uniform code and length of encoded sequences for coding with error probability at most $0.1$.
  }
...
\begin{enumerate}
  ...
  \item[22] Hamming weight of sequence $\vec x$ over the alphabet $\{0, \dots ,q-1\}$ is defined as the number of non-zero elements in $\vec x$. How many different binary sequences of length $n$ and $w$ exist?
...
\begin{itemize}
  ...
  \footnotesize {
  \item \emph{hint:} There are $A_n^w$ different choices of $w$ elements from the set of n elements (the order pf choosing is taken into account):
  \[
...
\begin{enumerate}
  ...
  \item[24] BMS $X=\{0,1\}$ generates '$1$' with probability $p=0,02$. Consider the dependence between error probability and length of encoded blocks, with the code rate $0.5$ bit for a letter of source.
...
\begin{enumerate}
  ...
  Compute the the number of ternary sequences of length $n$ contain $\tau_0$, $\tau_1$ and $\tau_2 =n-\tau_0- \tau_1$ elements of $0$, $1$ and $2$ respectively? 
